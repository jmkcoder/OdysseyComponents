...
datePicker.addEventListener('date-change', (event) => {
  ...
  // For events on the selected date
  if (event.detail.hasEvents) {
    console.log('Events:', event.detail.events);
  }
  
  // Source of the change ('calendar-selection', 'manual-input', etc.)
...
// Date clear event
datePicker.addEventListener('date-clear', () => {
...
|--------------------|---------------------------------------------------|----------------------------------------------|
| `date-change`      | Single mode: `{date: string, dateObj: Date, events: string[], hasEvents: boolean, source: string}`<br/>Range mode: `{rangeStart: string, rangeEnd: string, availableDates: string[], availableDatesObjects: Date[], source: string}` | Fired when the selected date changes |
